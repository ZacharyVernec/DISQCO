...
def plot_results():
    ...
    folderpath = Path(f'./results_massive_qft/')

    # Initialize data storage
    qpu_counts = {method:[] for method in method_names}
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                if "num_partitions" in line:
                    qpu_count = int(line.split(",")[2].split("=")[1].strip())
                    qpu_counts[method_name].append(qpu_count)
    ...
    for i in range(len(method_names)-1):
        assert qpu_counts[method_names[i]] == qpu_counts[method_names[i+1]], f"Qubit counts do not match across methods {method_names[i]} and {method_names[i+1]}"
    qpu_counts = qpu_counts[method_names[0]]
    x = np.arange(len(qpu_counts))

    # Plotting
    width = 1 / (len(method_names)+1)  # the width of the bars
    figsize=(width*len(qpu_counts)*20, 5)
    ...
    plt.title("E-bit Count vs Number of QPUs (for 256 qubits)")
    plt.xticks(x, qpu_counts)
    plt.legend(loc='upper left', ncols=len(method_names))
    plt.legend()
    plt.savefig(folderpath / "bar_chart_e_bit_count_vs_num_qpus.png")
    ...
    plt.title("Time vs Number of QPUs (for 256 qubits)")
    plt.xticks(x, qpu_counts)
    plt.legend(loc='upper left', ncols=len(method_names))
    plt.legend()
    plt.savefig(folderpath / "bar_chart_time_vs_num_qpus.png")
